...
export default class PullRefreshScrollView extends Component {
    ...
    onTouchMove(e){
      ...
      let y = target.pageY;//target.contentOffset.y;
      let x = target.pageX;
      if (this.dragFlag && this.touchStartY < 300 && Math.abs(this.touchStartX - x) < 5) {
        if ( (this.touchStartY - y) <= -10) {
          this.upState();
        } else if((this.touchStartY - y) < -5){
          this.downState();
        }
      }
    ...
    // 手指离开
    onScrollEndDrag(e){
      //this.touchEndY = e.nativeEvent.pageY
      this.dragFlag = false;
      ...
      }else if(this.isRefresh === 'downState'){
          Animated.timing(this.state.height,{
              toValue: 0,
              duration: 100,
              easing: Easing.inOut(Easing.quad)
