...
import HomeScrollView from './component/homeScrollView';
...
export default class Home extends Component {
    ...
    componentWillMount(){
        if(__scenes.length == 1){
			...
			EventEmitter.addListener('changeStatusBar',(data) => {
                if(this.view){
                    //console.log('changeStatusBar-home', data)
                    this.setState({
                        statusBar: data
                    })
                }
    ...
    growingIO(n){
        switch (n){
            case 0:
                ...
                break;
            case 1:
                Grow.track('elbn_home_checkin_click',{'elbn_home_signin_click':'每日签到图标点击量'});  //每日签到图标点击量
                break;
            case 2:
    ...
    componentDidMount(){
        ...
        if(global.__toLogin ){
            ...
            setTimeout(() => {
                this.props.navigation.navigate('register',{fromLaunch: true})
            },100);
        }
        if(__scenes.length == 1) {
             this.checkVersion();
            ...
            Storage.getItem(StorageKeys.eTD_USERINFO).then(res => {
                ...
                if (res && res.sessionId) {
                    this.setState({
                        ...
                        (!this.isLoginChange || flag) && this.getHomeData();
    ...
    getHomeData(){
        ...
        body = this.state.isLogin ? {
		 ...
		 Fetch.post('more/ind',body,(res) => {
            ...
            NativeModules.ETDHomepageView.onPullRefreshFinished();
            if (res.success) {
                let reg = /(?=(?!\b)(\d{4})+$)/g;
                this.setState({
                    homeData : res.body,
    ...
    _loadAdInfos(){
        Fetch.post('more/getSuspensionPosition', {}, res => {
					console.log('getSuspensionPosition', res)
            if(res && res.success){
                this.setState({
                    adInfos: res.body
                })
    ...
    render() {
        return (
            <View ref={ref => this.view=ref} style={homePageStyle.topLevelContainer}>
                ...
                <HomeScrollView
                    style={{flex:1}}
                    onPullRefresh={this._refresh.bind(this)}
                    contentHeight={Util.pixel * (this.state.isLogin ? this.state.isInvested ? 950 : 1160 : 1290) + ''}
                    refreshTitle={this.state.pullRefreshText}
                    >
                    <View style={homePageStyle.container} >
                        <View style={{backgroundColor: '#fff',position: 'relative',zIndex: 10}}>
                            {
                                            ...
                                            <View style={[homePageStyle.productLists,{paddingBottom: Util.pixel*22}]}>
                                                ...
                                                <View style={{paddingBottom: Util.pixel*8}}>
                                                    ...
                                                    <View style={[homePageStyle.flexRow,{justifyContent: 'center'}]}>
                                                        <Button buttonName="立即出借" color="#e94639" width={Util.size.width - Util.pixel*74} onPress={this.toPage.bind(this,'projectInfo',{useType: 0,claId: this.state.homeData.iteList[0].claId,iteId: this.state.homeData.iteList[0].iteId,title: '项目详情',noLine: true})} />
                                                    </View>
                                                </View>
                                            </View>
                                            : null
